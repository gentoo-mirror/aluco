...
LANGS="en ar bg ca cs de el es es_ES es_MX fa fi fr id it ja ne pl pt_BR ru sr sv uk zh_CN zh_TW"
for X in ${LANGS} ; do
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${P}-opencollada-debug.patch

	# fix undeclared false in jp2.c
	epatch "${FILESDIR}"/${P}-jp2.patch
}
...
src_install() {
	...
	if [[ -z ${LINGUAS} || ${LINGUAS} == "en" ]]; then
		rm -r "${WORKDIR}/install/${PV}/datafiles/locale"
	else
		for x in "${WORKDIR}"/install/${PV}/datafiles/locale/* ; do
			mylang=${x##*/}
